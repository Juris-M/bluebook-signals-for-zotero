...
# Go through the release steps for bluebook-signals-for-zotero
...
### Build the xpi
echo -n "Building bluebook-signals-for-zotero.xpi ... "
. ./build.sh
echo done
...
if [ "${ALL_CHECKED_IN}" == "1" ]; then
  ...
  SHA1=$(openssl sha1 bluebook-signals-for-zotero.xpi | sed -e 's/.*=[[:space:]]*\([^ ]\+\).*/\1/')
